...
public class Network_game_server_player extends AppCompatActivity implements View.OnClickListener {
        ...
        if (arrow_pressed == false) {
            final Runnable rotation = new Runnable() {
                public void run() {
                    ...
                    tableLayout.animate().setDuration(duration).rotationBy(degree_of_rotation).withEndAction(move);
    ...
    private void win_animate() {
        ...
        Runnable translation = new Runnable() {
            public void run() {
                imageView.animate().setInterpolator(new OvershootInterpolator()).setDuration(600).translationYBy(510);
                textView.animate().setInterpolator(new OvershootInterpolator()).setDuration(600).translationYBy(510);
            }
        };
        imageView.animate().setDuration(400).alphaBy(100).withEndAction(translation);
